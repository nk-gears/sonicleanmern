...
/* GET dealers list */
router.get('/dealers', passport.authenticate('jwt', { session: false }), (req, res) => {
    ...
    User.countDocuments(query).then(totalCount=> {
        ...
        .then(response=>{
            ...
            var response = {data : response, pages: totalPages, currentPage: _pageNumber, sizePerPage: _pageSize };
            res.json(response)
        })
    })
