...
const Payment = ({
  ...
  const onSubmitOrder = async () => {
    if (selectedCard.length === 0) {
      ...
      toast.error('Please input the employee name', {
        position: 'bottom-right',
      });
      return;
    } else if (selectedUsers.length === 0) {
      toast.error('Please select one more Employee', {
        position: 'bottom-right',
      });
      return;
    } else if (orderType === 1 && ship.length === 0) {
      toast.error('Please select one more product', {
        position: 'bottom-right',
      });
      return;
    ...
    if (orderType === 1 || orderType === 2) {
      ...
      data.order.items = ship.map(item => {
        ...
        p.item = item;
        p.quantity = 1;
        let result = Contants.DirectShipProducts.filter(
      ...
      for (const key of Object.keys(map)) {
        let p = {};
        p.item = key;
        let result = Contants.InventoryProducts.filter(
