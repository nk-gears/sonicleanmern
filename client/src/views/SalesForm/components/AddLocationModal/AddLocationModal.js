...
const validationSchema = function(values) {
  return Yup.object().shape({
    ...
    phonenumber: Yup.string().required('Store phone number is required'),
    us_state: Yup.string().required('state is required'),
    accept: Yup.bool()
      .required('* required')
      .test(
        'accept',
...
class AddLocationModal extends Component {
  ...
  handleBlur = () => {
    if (this.state.us_state === null) {
      ...
      if (
        this.state.us_state.length > 0 ||
        this.state.us_state.length === undefined
      ) {
                            ...
                            <Field
                              ...
                              render={({ field }) => (
                                <Select
                                  ...
                                  invalid={
                                    touched.us_state && !!errors.us_state
                                  }
                                  onChange={this.saveChanges}
                                  onBlur={() => this.handleBlur()}
                                  className={classNames(
                                    this.state.us_state_error
                                      ? 'error-select'
                                      : ''
                                  )}
                                />
