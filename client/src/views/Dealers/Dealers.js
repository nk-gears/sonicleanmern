...
const Dealers = ({ 
    ...
    const actionFormatter  = (cell, row) => {
        return (
            <>
                <Button onClick={() => history.push(`/sales/${row._id}`)} color="primary" size="sm">Order</Button>
                <Button onClick={() => history.push(`/profile/account/${row._id}`)} className="ml-1" color="success" size="sm">Detail</Button>
                <Button className="ml-1" color="danger" size="sm">Delete</Button>
            </>
        )
    }
    ...
    const onPageChange = (page, sizePerPage) => {
        ...
        getDealersList(page, sizePerPage, email)
    }

    const verifiedFormatter = (cell, row) => {
        if(cell) {
            return  <h6><Badge color="success" pill>Verified</Badge></h6>
        } else {
            return <h6><Badge color="danger" pill>UnVerified</Badge></h6>
        }
    }
                ...
                <CardBody>
                    {
                        ...
                        >
                            <TableHeaderColumn isKey={true} dataField="firstName" dataFormat={nameFormatter} dataSort>Name</TableHeaderColumn>
                            <TableHeaderColumn dataField="email" dataSort>Email</TableHeaderColumn>
                            <TableHeaderColumn dataField="companyName" dataAlign='right' dataSort>Company</TableHeaderColumn>
                            <TableHeaderColumn dataField="mohawkAccount" dataAlign='right' dataSort  width="10%">Mohawk Account</TableHeaderColumn>
                            <TableHeaderColumn dataField="mohawkBrand" dataAlign='right' dataSort  width="10%">mohawk Brand</TableHeaderColumn>
                            <TableHeaderColumn dataField="isVerified" dataFormat={verifiedFormatter} dataSort dataAlign='right'  width="10%">Verified</TableHeaderColumn>
...
const mapStateToProps = ({ official, account }) => {
    ...
    return { dealersList, totalCount, state, currentPage, sizePerPage, accountData };
}
