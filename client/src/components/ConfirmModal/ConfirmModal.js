...
const ConfirmModal = ({
  ...
  const onDelete = async id => {
    switch (type) {
      case 'storeDelete':
        await onDeleteStore(id, dealer);
        break;
      case 'cardDelete':
        await onDeleteCard(id, dealer);
        break;
      case 'userDelete':
        await onDeleteUser(id, dealer);
        break;
      default:
        return '';
    }
    ...
    <>
      ...
      <Modal isOpen={modal} className={'modal-primary modal-md'}>
        ...
        <ModalBody className="text-center">
          ...
          <Row>
            <Col>
              <Button color="primary m-1 w-100" onClick={() => onDelete(id)}>
                Yes
              </Button>
            </Col>
            <Col>
...
export default connect(mapStateToProps, mapDispatchToProps, null)(ConfirmModal);
